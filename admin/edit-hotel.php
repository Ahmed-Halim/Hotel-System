...
function SaveChanges() {
  ...
  edit_hotel($_POST["id"], $_POST["name"], $_POST["city"], $_POST["description"], $_POST["google_map"], $_POST["single_price"], $_POST["double_price"], $_POST["suit_price"], $_POST["hotel_manager"]);
}

function edit_hotel($hotel_id, $name, $city, $description, $google_map, $single_price, $double_price, $suit_price, $hotel_manager) {
  global $DB, $success, $failed;
  $stmt = $DB->prepare("UPDATE hotel SET name = ?, city = ?, description = ?, google_map = ?, single_price = ?, double_price = ?, suit_price = ?, manager_id = ? WHERE hotel_id = ?");
...
function getHotel($id) {
  ...
  if ($row = $stmt->fetch()) {
    ...
    $Hotel["images"] = array();
    $Hotel["hotel_manager"] = $row["manager_id"];
...
function getUsers() {
  ...
  while ($row = $stms->fetch()) {
    $selected = '';
    if ($row["user_id"] == $Hotel["hotel_manager"]) {
      $selected = ' selected';
    }
    $res .= '
...
<body>
  ...
  <div id="main">
    ...
    <form action="edit-hotel.php" method="POST" enctype="multipart/form-data">
      ...
      <label>City <input type="text" name="city" placeholder="Hotel City .." value="<?php if(isset($Hotel["city"])) echo $Hotel["city"]; ?>"/></label>
